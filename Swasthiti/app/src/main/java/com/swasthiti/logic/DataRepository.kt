...
import com.swasthiti.logic.FirebaseManager
...
object DataRepository {
    ...
    private val _provisionalAnalysis = MutableStateFlow<com.swasthiti.models.DailyReport?>(null)
    val provisionalAnalysis: StateFlow<com.swasthiti.models.DailyReport?> = _provisionalAnalysis
    ...
    fun initWithDb(context: Context, userId: String) {
        ...
        dbInitialized = true
        val db = SwasthitiDatabase.getInstance(context.applicationContext)
    ...
    private val _userProfile = MutableStateFlow<com.swasthiti.models.UserProfile?>(null)
    val userProfile: StateFlow<com.swasthiti.models.UserProfile?> = _userProfile

    private val _firstLoginComplete = MutableStateFlow(false)
    val firstLoginComplete: StateFlow<Boolean> = _firstLoginComplete

    // User role: "user" or "parent" — determines app experience
    ...
    private val _isDatabaseLinked = MutableStateFlow(false)
    val isDatabaseLinked: StateFlow<Boolean> = _isDatabaseLinked
    ...
    fun init(context: Context) {
        ...
        prefs = context.getSharedPreferences("Swasthiti_data_store", Context.MODE_PRIVATE)
        ...
        if (savedEmail.isNotEmpty()) {
            _userProfile.value = com.swasthiti.models.UserProfile(
                email = savedEmail,
        ...
        // Check if app is linked to Firebase Firestore
        CoroutineScope(Dispatchers.IO).launch {
            Log.d(TAG, "🔍 [TRACE 1] DataRepository: Requesting connection check...")
            val result = FirebaseManager.checkDatabaseLinkage()
            Log.d(TAG, "🔍 [TRACE 5] DataRepository: Final link status = $result")
            _isDatabaseLinked.value = result
        }
    }

    // --- Mutators ---

    fun saveUserProfile(profile: com.swasthiti.models.UserProfile) {
    ...
    /** Persist the user's role selection ("user" or "parent"). Called once during onboarding. */
    ...
    fun updateProvisionalAnalysis(result: com.swasthiti.models.DailyReport?) {
