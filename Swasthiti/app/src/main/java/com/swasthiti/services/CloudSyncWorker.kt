...
class CloudSyncWorker(appContext: Context, workerParams: WorkerParameters) :
    ...
    companion object {
        private const val TAG = "Swasthiti.CloudSync"
    ...
    override suspend fun doWork(): Result {
        ...
        val db = SwasthitiDatabase.getInstance(applicationContext)

        try {
            Log.d(TAG, "=== CloudSyncWorker started for user: ${user.email} (uid: $uid) ===")

            // 1. Validate Active Device ID
            val prefs = applicationContext.getSharedPreferences("Swasthiti_prefs", Context.MODE_PRIVATE)
