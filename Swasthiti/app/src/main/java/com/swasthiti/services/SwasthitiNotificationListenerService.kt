...
import com.swasthiti.logic.db.NotificationEventEntity
 ...
 *
 * User setup: Settings → Notifications → Notification access → Swasthiti → Allow.
 */
class SwasthitiNotificationListenerService : NotificationListenerService() {

    companion object {
        private const val TAG = "Swasthiti.NLS"
    }
    ...
    private fun persistEvent(entity: NotificationEventEntity) {
        serviceScope.launch {
            try {
                val db = SwasthitiDatabase.getInstance(this@SwasthitiNotificationListenerService)
