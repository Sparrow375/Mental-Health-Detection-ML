...
class ImprovedAnomalyDetector:
    ...
    def __init__(self, baseline: PersonalityVector):
        ...
        # u33 (PHQ=25, peak_ev=2.710) is the lowest TP GÇö threshold sits just below it.
        ...
        # Secondary watch threshold GÇö flags borderline cases for human review
    ...
    def calibrate_from_baseline(self, baseline_df: pd.DataFrame):
        ...
        if baseline_df is None or len(baseline_df) < 7:
            return  # Not enough data GÇö keep constructor defaults
    ...
    def _generate_notes(self, anomaly_score: float, alert_level: str, pattern_type: str) -> str:
        ...
        if self.sustained_deviation_days >= self.SUSTAINED_THRESHOLD_DAYS:
            notes.append(f"GÜá Sustained deviation detected ({self.sustained_deviation_days} consecutive days)")
        ...
        if anomaly_score > 0.6 and alert_level == 'green':
            notes.append("Gä¦ High single-day score but no sustained pattern yet")
...
class ReportGenerator:
    ...
    def generate_pdf(self, baseline_df, monitoring_df, daily_reports, final_prediction):
        ...
        print(f"  G£ô Saved PDF Report: {filename}")
        return filename
...
def run_scenario(scenario: str, patient_id: str):
    ...
    print(f"  G£ô Baseline established")
    ...
    for level in ['green', 'yellow', 'orange', 'red']:
        ...
        bar = 'Gûê' * (count // 4) # Adjust bar scale for 180 days
...
def main():
    ...
    comparison_lines.append("GöÇ" * 80)
    ...
    print(f"{'='*80}")
    print(f"\nGenerated Artifacts:")
    print(f"  GÇó Clinical PDF Reports: report_*.pdf")
    print(f"  GÇó Overview PNG Charts: analysis_*.png")
    print(f"  GÇó Comparative Summary: comparison_summary.txt")
